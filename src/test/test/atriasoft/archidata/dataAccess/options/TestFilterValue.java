...
class TestFilterValue {
	...
	@Order(2)
	@Test
	void testPartialUpdateWhitelist() throws Exception {
		final Model update = new Model();
		update.field1 = "modified1";
		update.field2 = "modified2";
		update.field3 = 999;
		ConfigureDb.da.updateById(update, idOfTheObject, new FilterValue("field1"));
		...
		Assertions.assertEquals(100, retrieved.field3);
	}

	@Order(3)
	@Test
	void testPartialUpdateMultipleFields() throws Exception {
		final Model update = new Model();
		update.field1 = "multi1";
		update.field2 = "multi2";
		update.field3 = 777;
		ConfigureDb.da.updateById(update, idOfTheObject, new FilterValue("field2", "field3"));

		final Model retrieved = ConfigureDb.da.getById(Model.class, idOfTheObject);
		Assertions.assertNotNull(retrieved);
		Assertions.assertEquals("modified1", retrieved.field1);
		Assertions.assertEquals("multi2", retrieved.field2);
		Assertions.assertEquals(777, retrieved.field3);
	}
}
