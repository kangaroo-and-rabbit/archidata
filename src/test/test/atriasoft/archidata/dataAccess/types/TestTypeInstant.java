...
class TestTypeInstant {
	...
	void testInsertAndRetrieve() throws Exception {
		...
		// Truncate to millis since MongoDB Date has millisecond precision
		test.data = Instant.now().truncatedTo(java.time.temporal.ChronoUnit.MILLIS);
	...
	void testEpoch() throws Exception {
		final Model test = new Model();
		test.data = Instant.EPOCH;
		final Model inserted = ConfigureDb.da.insert(test);

		final Model retrieved = ConfigureDb.da.getById(Model.class, inserted.id);
		Assertions.assertNotNull(retrieved);
		Assertions.assertEquals(Instant.EPOCH, retrieved.data);
	...
	void testUpdate() throws Exception {
		final Model test = new Model();
		test.data = Instant.parse("2020-01-01T00:00:00Z");
		final Model inserted = ConfigureDb.da.insert(test);

		inserted.data = Instant.parse("2025-06-15T12:30:00Z");
		ConfigureDb.da.updateById(inserted, inserted.id);

		final Model retrieved = ConfigureDb.da.getById(Model.class, inserted.id);
		Assertions.assertNotNull(retrieved);
		Assertions.assertEquals(Instant.parse("2025-06-15T12:30:00Z"), retrieved.data);
