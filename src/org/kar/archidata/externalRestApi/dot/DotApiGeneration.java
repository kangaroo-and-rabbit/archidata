...
public class DotApiGeneration {
	...
	public static String generateClassObjectModelTypescript(
		...
		if (writeMode) {
			return dotModel.dotTypeName + "Write";
		}
		return dotModel.dotTypeName;
	}
	...
	public static String generateClassMapModelTypescript(
		...
		out.append("{[key: ");
		out.append(generateClassModelTypescript(model.keyModel, dotGroup, imports, writeMode));
		out.append("]: ");
		out.append(generateClassModelTypescript(model.valueModel, dotGroup, imports, writeMode));
		out.append(";}");
		return out.toString();
	}
	...
	public static String generateClassModelTypescript(
		...
		if (model instanceof final ClassObjectModel objectModel) {
			return generateClassObjectModelTypescript(objectModel, dotGroup, imports, writeMode);
		}
		if (model instanceof final ClassListModel listModel) {
			return generateClassListModelTypescript(listModel, dotGroup, imports, writeMode);
		}
		if (model instanceof final ClassMapModel mapModel) {
			return generateClassMapModelTypescript(mapModel, dotGroup, imports, writeMode);
		}
		if (model instanceof final ClassEnumModel enumModel) {
			return generateClassEnumModelTypescript(enumModel, dotGroup, imports, writeMode);
		}
	...
	public static String generateClassModelsTypescript(
		...
		for (final ClassModel model : models) {
			...
			final String data = generateClassModelTypescript(model, dotGroup, imports, writeMode);
			out.append(data);
		}
		return out.toString();
	}
	...
	public static void generateApiFile(
		...
		for (final ApiModel interfaceElement : element.interfaces) {
			...
			final String returnComplexModel = DotClassElement.generateLocalModel(returnModelNameIfComplex,
					interfaceElement.returnTypes, dotGroup);
			...
			if (interfaceElement.unnamedElement.size() == 1) {
				data.append("\n\t\tdata: ");
				data.append(generateClassModelTypescript(interfaceElement.unnamedElement.get(0), dotGroup, writeImports,
						true));
				data.append(",");
				...
				for (final Entry<String, List<ClassModel>> pathEntry : interfaceElement.multiPartParameters
					...
					data.append(": ");
					data.append(generateClassModelsTypescript(pathEntry.getValue(), dotGroup, writeImports, true));
					data.append(",");
				}
				data.append("\n\t\t},");
			}
			...
			if (returnComplexModel != null) {
				...
				final String returnType = generateClassModelsTypescript(interfaceElement.returnTypes, dotGroup, imports,
						false);
		...
		final String fileName = DotClassElement.determineFileName(element.name);
